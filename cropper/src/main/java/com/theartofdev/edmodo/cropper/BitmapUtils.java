...
final class BitmapUtils {
  ...
  static BitmapSampled decodeSampledBitmap(Context context, Uri uri, int reqWidth, int reqHeight) {
    ...
    try {
      ...
      // First ffmpeg_decoder_decoding with inJustDecodeBounds=true to check dimensions
  ...
  private static BitmapSampled cropBitmap(
    ...
    int sampleSize = 1;
    try {
      // ffmpeg_decoder_decoding only the required image from URI, optionally sub-sampling if reqWidth/reqHeight is
      // given.
    ...
    if (result != null) {
      ...
      // failed to decode region, may be skia issue, try full ffmpeg_decoder_decoding and then crop
      return cropBitmap(
          context,
          loadedImageUri,
          points,
  ...
  private static Bitmap decodeImage(
    ...
    throw new RuntimeException("Failed to ffmpeg_decoder_decoding image: " + uri);
  }
  ...
  private static int getMaxTextureSize() {
    ...
    try {
      ...
      for (int i = 0; i < totalConfigurations[0]; i++) {
        // Only need to check for width since opengl gl_textures are always squared
