...
final class CropWindowHandler {
    ...
    private CropWindowHandlerManager mManager;

    public CropWindowHandler(CropWindowHandlerManager manager) {
        mManager = manager;
    }
...
//  private float mScaleFactorHeight = 1;
    ...
    //    return Math.min(mMinCropWindowWidth, mMinCropResultWidth / mScaleFactorWidth);
        return mManager.getMinCropWidth();
    }

    /** Minimum height in pixels that the crop window can get. */
    public float getMinCropHeight() {
        return mManager.getMinCropHeight();
...
//  }
    ...
    /**
     * Determines which, if any, of the handles are pressed given the touch coordinates, the
     * bounding box/oval, and the touch radius.
     *
     * @param x
     *         the x-coordinate of the touch point
     * @param y
