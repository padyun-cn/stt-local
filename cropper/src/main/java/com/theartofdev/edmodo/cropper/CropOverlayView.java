...
public class CropOverlayView extends View {
    ...
    private final CropWindowHandlerManager mCropWindowHandlerManager = new CropWindowHandlerManager();
    ...
    public RectF getCropWindowRect() {
        return mCropWindowHandlerManager.getRect();
    }

    /** Get the left/top/right/bottom coordinates of the crop window. */
    public List<RectF> getAllCropWindowRects() {
        return mCropWindowHandlerManager.getAllRects();
    }

    /** Get the left/top/right/bottom coordinates of the crop window. */
    public List<RectF> getUnfocusedCropWindowRects() {
        return mCropWindowHandlerManager.getUnfocusedRects();
    }

    /** Get the left/top/right/bottom coordinates of the crop window. */
    public void foreachUnfocusedCropWindowRects(Consumer<RectF> consumer) {
         mCropWindowHandlerManager.foreachUnfocusedRects(consumer);
    }

    /** Set the left/top/right/bottom coordinates of the crop window. */
    public void setCropWindowRect(RectF rect) {
        LogUtil.d("nimada#handler", "setCropWindowRect::156::" + rect.toShortString());
        mCropWindowHandlerManager.setRect(rect);
    }

    /* ADDED BY DAIPENGFEI on 2020-02-15. */
    /** Set the left/top/right/bottom coordinates of the crop window. */
    public void setCropWindowRect(int index, RectF rect) {
        mCropWindowHandlerManager.setRect(index, rect);
    }
    /* ADDED END. */
     ...
     *
     * @param boundsPoints
     *         the image's bounding points
     * @param viewWidth
     *         The bounding image view width.
     * @param viewHeight
     *         The bounding image view height.
     */
    public void setBounds(float[] boundsPoints, int viewWidth, int viewHeight) {
        if (boundsPoints == null || !Arrays.equals(mBoundsPoints, boundsPoints)) {
            ...
            RectF cropRect = mCropWindowHandlerManager.getRect();
    ...
    public void resetCropOverlayView() {
        if (initializedCropWindow) {
            LogUtil.d("nimada#handler", "/reset::196/");
    ...
    public void setMinCropResultSize(int minCropResultWidth, int minCropResultHeight) {
        mCropWindowHandlerManager.setMinCropResultSize(minCropResultWidth, minCropResultHeight);
    }
    ...
    public void setMaxCropResultSize(int maxCropResultWidth, int maxCropResultHeight) {
        mCropWindowHandlerManager.setMaxCropResultSize(maxCropResultWidth, maxCropResultHeight);
    }
    ...
    public void setCropWindowLimits(
        ...
        mCropWindowHandlerManager.setCropWindowLimits(
                maxWidth, maxHeight, scaleFactorWidth, scaleFactorHeight);
    }

    /* ADDED BY DAIPENGFEI on 2020-02-16. */

    public Rect folkRect(int offset){
        return mCropWindowHandlerManager.folk(offset);
    }

    public boolean defolk(){
        boolean r = mCropWindowHandlerManager.defolk();
        invalidate();
        return r;
    }
    /* ADDED END. */
    ...
    public void resetCropWindowRect() {
        mCropWindowHandlerManager.reset();
        if (initializedCropWindow) {
            initCropWindow();
            invalidate();
            callOnCropWindowChanged(false);
    ...
    /**
     * Sets all initial values, but does not call initCropWindow to reset the views.<br> Used once
     * at the very start to initialize the attributes.
     */
    public void setInitialAttributeValues(CropImageOptions options) {

        mCropWindowHandlerManager.setInitialAttributeValues(options);
        ...
        /* ADDED BY DAIPENGFEI on 2020-02-15. */
        mUnfocusedPaint = getNewPaintOrNull(2, Color.RED);
        if(mUnfocusedPaint != null) {
            mUnfocusedPaint.setPathEffect(new DashPathEffect(new float[]{4, 4}, 0));
        }
        /* ADDED END. */
    }
    ...
    private void initCropWindow() {
        ...
        if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) {
            ...
            rect.left =
                    leftLimit + mInitialCropWindowRect.left / mCropWindowHandlerManager.getScaleFactorWidth();
            rect.top = topLimit + mInitialCropWindowRect.top / mCropWindowHandlerManager.getScaleFactorHeight();
            rect.right =
                    rect.left + mInitialCropWindowRect.width() / mCropWindowHandlerManager.getScaleFactorWidth();
            rect.bottom =
                    rect.top + mInitialCropWindowRect.height() / mCropWindowHandlerManager.getScaleFactorHeight();
            ...
            if (bitmapAspectRatio > mTargetAspectRatio) {
                ...
                float cropWidth =
                        Math.max(mCropWindowHandlerManager.getMinCropWidth(), rect.height() * mTargetAspectRatio);
                ...
                // Limits the aspect ratio to no less than 40 wide or 40 tall
                float cropHeight =
                        Math.max(mCropWindowHandlerManager.getMinCropHeight(), rect.width() / mTargetAspectRatio);
    ...
    private void fixCropWindowRectByRules(RectF rect) {
        RectF clone = new RectF(rect);

        if (rect.width() < mCropWindowHandlerManager.getMinCropWidth()) {
            float adj = (mCropWindowHandlerManager.getMinCropWidth() - rect.width()) / 2;
            rect.left -= adj;
            rect.right += adj;
        }
        if (!rect.equals(clone)) {
            LogUtil.d("nimada", "fixCurrentRect::527::" + rect.toShortString() + " /clone:: " + clone.toShortString());
        }
        if (rect.height() < mCropWindowHandlerManager.getMinCropHeight()) {
            float adj = (mCropWindowHandlerManager.getMinCropHeight() - rect.height()) / 2;
            rect.top -= adj;
            rect.bottom += adj;
        }
        if (!rect.equals(clone.toShortString())) {
            LogUtil.d("nimada", "fixCurrentRect::536::" + rect.toShortString() + " /clone:: " + clone.toShortString());
        }
        if (rect.width() > mCropWindowHandlerManager.getMaxCropWidth()) {
            float adj = (rect.width() - mCropWindowHandlerManager.getMaxCropWidth()) / 2;
            rect.left += adj;
            rect.right -= adj;
        }
        if (!rect.equals(clone)) {
            LogUtil.d("nimada", "fixCurrentRect::544::" + rect.toShortString() + " /clone:: " + clone.toShortString());
        }
        if (rect.height() > mCropWindowHandlerManager.getMaxCropHeight()) {
            float adj = (rect.height() - mCropWindowHandlerManager.getMaxCropHeight()) / 2;
            rect.top += adj;
            rect.bottom -= adj;
        }
        if (!rect.equals(clone)) {
            LogUtil.d("nimada", "fixCurrentRect::552::" + rect.toShortString() + " /clone:: " + clone.toShortString());
        }
        calculateBounds(rect);
        if (!rect.equals(clone)) {
            LogUtil.d("nimada", "fixCurrentRect::554::" + rect.toShortString() + " /clone:: " + clone.toShortString());
        }
        ...
        if (!rect.equals(clone)) {
            LogUtil.d("nimada", "fixCurrentRect::577::" + rect.toShortString() + " /clone:: " + clone.toShortString());
        }
        ...
        if (!rect.equals(clone)) {
            LogUtil.d("nimada", "fixCurrentRect::592::" + rect.toShortString() + " /clone:: " + clone.toShortString());
        }
    }

    /**
     * Draw crop overview by drawing background over image not in the cripping area, then borders
     * and guidelines.
     */
    @Override
    protected void onDraw(Canvas canvas) {
        ...
        if (mCropWindowHandlerManager.showGuidelines()) {
...
//    RectF r = new RectF(11, 8, 102, 100);
    /** Draw shadow background over the image not including the crop area. */
    private void drawBackground(Canvas canvas) {

        RectF rect = mCropWindowHandlerManager.getRect();
...
//        RectF rr = new RectF(r);
        //rr.inset(rr.width() - mTargetAspectRatio * rr.width(), rr.height() - mTargetAspectRatio * rr.height());
        //rr.offsetTo(left + 11 * getAspectRatioX(), top + 8 * getAspectRatioY());
        //rr.offsetTo(left + rr.left, top + rr.top);

        /* ADDED BY DAIPENGFEI on 2020-02-15. */
        foreachUnfocusedCropWindowRects(rectF -> canvas.drawRect(rectF, mUnfocusedPaint));
        /* ADDED END. */

        if (mCropShape == CropImageView.CropShape.RECTANGLE) {
            if (!isNonStraightAngleRotated() || Build.VERSION.SDK_INT <= 17) {
                LogUtil.d("mingcuiliu ", "rect " + rect.toShortString());
    ...
    /**
     * On press down start crop window movment depending on the location of the press.<br> if press
     * is far from crop window then no move handler is returned (null).
     */
    private void onActionDown(float x, float y) {
        mMoveHandler = mCropWindowHandlerManager.getMoveHandler(x, y, mTouchRadius, mCropShape);
        if (mMoveHandler != null) {
            invalidate();
            callOnCropOpratingStateChanged(CropOpratingStateChangedListener.START);
        }
    }
    ...
    private void onActionUp() {
        if (mMoveHandler != null) {
            mMoveHandler = null;
            callOnCropWindowChanged(false);
            callOnCropOpratingStateChanged(CropOpratingStateChangedListener.IDLE);
            invalidate();
        }
    }
     ...
     * float)}.<br> The move handler will do the proper move/resize of the crop window.
     */
    private void onActionMove(float x, float y) {
        if (mMoveHandler != null) {
            float snapRadius = mSnapRadius;
            RectF rect = mCropWindowHandlerManager.getRect();
            LogUtil.d("nimada", "onActionMove::888::" + rect.toShortString());
            if (calculateBounds(rect)) {
                snapRadius = 0;
            }
            LogUtil.d("nimada", "onActionMove::892::" + rect.toShortString());
            mMoveHandler.move(
                    rect,
                    x,
                    y,
                    ...
                    mFixAspectRatio,
                    mTargetAspectRatio);
            LogUtil.d("nimada", "onActionMove::903::" + rect.toShortString());
            mCropWindowHandlerManager.setRect(rect);
            callOnCropWindowChanged(true);
            callOnCropOpratingStateChanged(CropOpratingStateChangedListener.OPRATING);
            invalidate();
        }
    }

    /**
     * Calculate the bounding rectangle for current crop window, handle non-straight rotation
     * angles.
     * <br>
     * If the rotation angle is straight then the bounds rectangle is the bitmap rectangle,
     * otherwsie we find the max rectangle that is within the image bounds starting from the crop
     * window rectangle.
     *
     * @param rect
    ...
    private void callOnCropWindowChanged(boolean inProgress) {
        try {
            ...
            LogUtil.e("AIC", "Exception in crop window changed", e);
        }
    }
    // endregion
    ...
    public interface CropWindowChangeListener {
         ...
         *
         * @param inProgress
         *         is the crop window change operation is still in progress by user touch
         */
    ...
    /* ADDED BY DAIPENGFEI on 10/10/19. */
    /**
     *
     */
    private CropOpratingStateChangedListener mCropOpratingStateChangedListener;

    /**
     *
     */
    private int mCropOpratingCurrentState = CropOpratingStateChangedListener.IDLE;

    /**
     * @param newState
     */
    private void callOnCropOpratingStateChanged(int newState) {
        if (mCropOpratingStateChangedListener != null && newState != mCropOpratingCurrentState) {
            mCropOpratingCurrentState = newState;
            mCropOpratingStateChangedListener.onCropOprating(newState);
        }
    }
     ...
     * @param l
     */
    public void setCropOpratingStateChangedListener(CropOpratingStateChangedListener l) {
        this.mCropOpratingStateChangedListener = l;
    }

    /**
     *
     */
    public interface CropOpratingStateChangedListener {
        int IDLE = 0;
        int START = 1;
        int OPRATING = 2;
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (newLeft < newRight
                    && newTop <= newBottom
                    && newLeft >= 0
                    && newRight <= mCropWindowHandlerManager.getMaxCropWidth()
                    && newTop >= 0
                    && newBottom <= mCropWindowHandlerManager.getMaxCropHeight()) {
                LogUtil.d("nimada", "onScale::1100::" + rect.toShortString());
                rect.set(newLeft, newTop, newRight, newBottom);
                LogUtil.d("nimada", "onScale::1102::" + rect.toShortString());
                mCropWindowHandlerManager.setRect(rect);
                invalidate();
            }
