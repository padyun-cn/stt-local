...
public class CropImageView extends FrameLayout {
    ...
    /**
     * if to save bitmap on save instance state.<br> It is best to avoid it by using URI in setting
     * image for cropping.<br> If false the bitmap is not saved and if restore is required to view
     * will be empty, storing the bitmap requires saving it to file which can be expensive. default:
     * false.
     */
     ...
     * cropping image.<br> default: true, may disable for animation or frame transition.
     */
    private boolean mShowCropOverlay = true;

    /**
     * if to show progress bar when image async loading/cropping is in progress.<br> default: true,
     * disable to provide custom progress bar UI.
     */
    private boolean mShowProgressBar = true;

    /**
     * if auto-zoom functionality is enabled.<br> default: true.
     */
    ...
    /**
     * The shape of the cropping area - rectangle/circular.<br> To set square/circle crop shape set
     * aspect ratio to 1:1.
     */
    ...
    /* ADDED BY DAIPENGFEI on 10/14/19. */

    /**
     * Set auto-zoom functionality to enabled/disabled.
     */
    public void setAutoZoomEnabled(boolean autoZoomEnabled) {
        setAutoZoomEnabled(autoZoomEnabled, null);
    }

    /**
     * @param autoZoomEnabled
     * @param finnallyRunnable
     */
    public void setAutoZoomEnabled(boolean autoZoomEnabled, Runnable finnallyRunnable) {
        if (mAutoZoomEnabled != autoZoomEnabled) {
            mAutoZoomEnabled = autoZoomEnabled;
            handleCropWindowChanged(false, finnallyRunnable != null, finnallyRunnable);
            mCropOverlayView.invalidate();
        } else /* ADDED BY DAIPENGFEI on 10/15/19. */if (finnallyRunnable != null) {
            new Handler().postDelayed(finnallyRunnable, 300);
        } /* ADDED END. */
    }
    /* ADDED END. */
    ...
    public void setFlippedHorizontally(boolean flipHorizontally) {
        if (mFlipHorizontally != flipHorizontally) {
            ...
            LogUtil.d("nimada#doapply", "fliph::574");
    ...
    public void setFlippedVertically(boolean flipVertically) {
        if (mFlipVertically != flipVertically) {
            ...
            LogUtil.d("nimada#doapply", "filpV::592");
            applyImageMatrix(getWidth(), getHeight(), true, false);
        }
    }
    ...
    /**
     * Sets the both the X and Y values of the aspectRatio.<br> Sets fixed aspect ratio to TRUE.
     *
     * @param aspectRatioX
     ...
     * @return
     */
    public RectF getImageBitmapDrawingRect() {
        float[] mBoundsPoints = mImagePoints;
    ...
    /* ADDED BY DAIPENGFEI on 2020-02-17. */
    /**
     *
     * @return
     */
    public List<Rect> getAllCropRects() {
        List<Rect> result = new ArrayList<>();
        for(float[] f: getAllCropPoints()) {
            if(f != null && f.length == 8){
                result.add(getCropRect(f));
            }
        }
        return result;
    }
    ...
    /* ADDED BY DAIPENGFEI on 2020-02-17. */
    public Rect getCropRect(float[] points){
     ...
     * displayed in the CropImageView) using the original image rotation.<br> Note: the 4 points may
     * not be a rectangle if the image was rotates to NOT stright angle (!= 90/180/270).
     *
...
/* MODIFIED BY DAIPENGFEI on 2020-02-17. */
        ...
        LogUtil.d("nimada#getCropPoints", "cropWindowPoions:: " + cropWindowRect.toShortString());
        LogUtil.d("nimada#getCropPoints", "beforeInvert:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
        mImageMatrix.invert(mImageInverseMatrix);
        LogUtil.d("nimada#getCropPoints", "afterInvert:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
        ...
        /*return points;*/
        return getCropPoints(cropWindowRect);
    }

    /**
     *
     * @param cropWindowRect
     * @return
     */
    public float[] getCropPoints(RectF cropWindowRect) {

        if (cropWindowRect == null) return null;
     ...
     * @return
     */
    public List<float[]> getAllCropPoints() {
        List<RectF> cropRects = mCropOverlayView.getAllCropWindowRects();
        List<float[]> floats = new ArrayList<>();
        for (RectF rf : cropRects) {
            if(rf == null || rf.isEmpty()) continue;
            floats.add(getCropPoints(rf));
        }
        return floats;
    }
...
/* MODIFIED END. */
    ...
    /**
     * Set the crop window position and size to the given rectangle.<br> Image to crop must be first
     * set before invoking this, for async - after complete callback.
     *
     * @param rect
    ...
    /* ADDED BY DAIPENGFEI on 2020-02-16. */

    public boolean defolk() {
        boolean r = mCropOverlayView.defolk();
        //applyImageMatrix(getWidth(), getHeight(), true, true);
     ...
     * @return
     */
    public Rect folkNewCropRect() {
        Rect r = mCropOverlayView.folkRect(DensityUtils.dip2px(getContext(), 15));
        handleCropWindowChanged(false, true);
        mCropOverlayView.invalidate();
        return r;
    }
    ...
    public void resetCropRect() {
        ...
        LogUtil.d("nimada#doapply", "reset::906");
    ...
    /**
     * Gets the cropped image based on the current crop window.<br> Uses {@link
     * RequestSizeOptions#RESIZE_INSIDE} option.
     *
     * @param reqWidth
    ...
    /**
     * Gets the cropped image based on the current crop window.<br> The result will be invoked to
     * listener set by {@link #setOnCropImageCompleteListener(OnCropImageCompleteListener)}.
     */
    ...
    /**
     * Gets the cropped image based on the current crop window.<br> Uses {@link
     * RequestSizeOptions#RESIZE_INSIDE} option.<br> The result will be invoked to listener set by
     * {@link #setOnCropImageCompleteListener(OnCropImageCompleteListener)}.
     *
     * @param reqWidth
     *         the width to resize the cropped image to
     * @param reqHeight
    ...
    /**
     * Save the cropped image based on the current crop window to the given uri.<br> Uses JPEG image
     * compression with 90 compression quality.<br> The result will be invoked to listener set by
     * {@link #setOnCropImageCompleteListener(OnCropImageCompleteListener)}.
     *
     * @param saveUri
    ...
    /**
     * Save the cropped image based on the current crop window to the given uri.<br> The result will
     * be invoked to listener set by {@link #setOnCropImageCompleteListener(OnCropImageCompleteListener)}.
     *
     * @param saveUri
     *         the Android Uri to save the cropped image to
     * @param saveCompressFormat
     *         the compression format to use when writing the image
     * @param saveCompressQuality
    ...
    /**
     * Save the cropped image based on the current crop window to the given uri.<br> Uses {@link
     * RequestSizeOptions#RESIZE_INSIDE} option.<br> The result will be invoked to listener set by
     * {@link #setOnCropImageCompleteListener(OnCropImageCompleteListener)}.
     *
     * @param saveUri
    ...
    /**
     * Set the callback to be invoked when image async cropping image ({@link
     * #getCroppedImageAsync()} or {@link #saveCroppedImageAsync(Uri)}) is complete (successful or
     * failed).
     */
    ...
    /**
     * Sets a bitmap loaded from the given Android URI as the content of the CropImageView.<br> Can
     * be used with URI from gallery or camera source.<br> Will rotate the image by exif data.<br>
     *
     * @param uri
    ...
    /**
     * Rotates image by the specified number of degrees clockwise.<br> Negative values represent
    ...
    public void rotateImage(int degrees) {
        if (mBitmap != null) {
            ...
            LogUtil.d("nimada#rotateImage", "beforeInvert:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            mImageMatrix.invert(mImageInverseMatrix);
            LogUtil.d("nimada#roateImage", "afterinvert:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            ...
            LogUtil.d("nimada#doapply", "rotate::1269");
            applyImageMatrix(getWidth(), getHeight(), true, false);
            ...
            LogUtil.d("nimada#doapply", "rotate::1280");
            applyImageMatrix(getWidth(), getHeight(), true, false);
            ...
            mCropOverlayView.resetCropOverlayView();
            LogUtil.d("nimada#handler", "/rotate::1302/");
            mCropOverlayView.setCropWindowRect(BitmapUtils.RECT);
            LogUtil.d("nimada#doapply", "rotate::1304");
            applyImageMatrix(getWidth(), getHeight(), true, false);
            handleCropWindowChanged(false, false);
    ...
    public void flipImageHorizontally() {
        ...
        LogUtil.d("nimada#doapply", "flipImageVertically::1319");
        applyImageMatrix(getWidth(), getHeight(), true, false);
    }
    ...
    public void flipImageVertically() {
        ...
        LogUtil.d("nimada#doapply", "flipImageVertically::1327");
        applyImageMatrix(getWidth(), getHeight(), true, false);
    }
    ...
    /**
     * On complete of the async bitmap loading by {@link #setImageUriAsync(Uri)} set the result to
     * the widget if still relevant and call listener if set.
     *
     * @param result
    ...
    /**
     * Set the given bitmap to be used in for cropping<br> Optionally clear full if the bitmap is
     * new, or partial clear if the bitmap has been manipulated.
     */
    private void setBitmap(
        ...
        if (mBitmap == null || !mBitmap.equals(bitmap)) {
            ...
            LogUtil.d("nimada#doapply", "setBitmap::1404");
            applyImageMatrix(getWidth(), getHeight(), true, false);
    ...
    /**
     * Clear the current image set for cropping.<br> Full clear will also clear the data of the set
     * image like Uri or Resource id while partial clear will only clear the bitmap and recycle if
     * required.
     */
    private void clearImageInt() {

        // if we allocated the bitmap, opengl_render_release it as fast as possible
...
//        setImageBitmap(mBitmap);
    ...
    /**
     * Gets the cropped image based on the current crop window.<br> If (reqWidth,reqHeight) is given
     * AND image is loaded from URI cropping will try to use sample size to fit in the requested
     * width and height down-sampling if possible - optimization to get best size to quality.<br>
     ...
     *         the height to resize the cropped image to (see options)
     * @param options
     *         the resize method to use on the cropped bitmap
     * @param saveUri
     *         optional: to save the cropped image to
     * @param saveCompressFormat
     *         if saveUri is given, the given compression will be used for saving the image
     * @param saveCompressQuality
     *         if saveUri is given, the given quality will be used for the compression.
     */
    ...
    public Parcelable onSaveInstanceState() {
        ...
        LogUtil.d("nimada#savedInstance", "beforeInvert:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
        mImageMatrix.invert(mImageInverseMatrix);
        mImageInverseMatrix.mapRect(BitmapUtils.RECT);
        LogUtil.d("nimada#savedInstance", "afterInvert:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mLayoutWidth > 0 && mLayoutHeight > 0) {
            ...
            if (mBitmap != null) {
                LogUtil.d("nimada#doapply", "onLayout::1722");
                ...
                if (mRestoreCropWindowRect != null) {
                    if (mRestoreDegreesRotated != mInitialDegreesRotated) {
                        ...
                        LogUtil.d("nimada#doapply", "onLayout::1726");
                        applyImageMatrix(r - l, b - t, true, false);
                    }
                    mImageMatrix.mapRect(mRestoreCropWindowRect);
                    LogUtil.d("nimada#handler", "/restore::1722/");
                    ...
                    mSizeChanged = false;
                    LogUtil.d("nimada#doapply", "onLayout::1737");
                    handleCropWindowChanged(false, false);
                }
            } else {
     ...
     * boolean, Runnable)} in {@link #layout(int, int, int, int)}.
     */
    @Override
    ...
    private void handleCropWindowChanged(boolean inProgress, boolean animate) {
        handleCropWindowChanged(inProgress, animate, null);
    }

    /**
     * Handle crop window change to:<br> 1. Execute auto-zoom-in/out depending on the area covered
     * of cropping window relative to the available view area.<br> 2. Slide the zoomed sub-area if
     * the cropping window is outside of the visible view sub-area.
     * <br>
     *
     * @param inProgress
     *         is the crop window change is still in progress by the user
     * @param animate
     *         if to animate the change to the image matrix, or set it directly
     */
    private void handleCropWindowChanged(boolean inProgress, boolean animate, /* MODIFIED BY DAIPENGFEI on 10/14/19. */ Runnable onCropImageAnimationEnd /* MODIFIED END. */) {
        int width = getWidth();
        ...
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            if (inProgress) {
                if (cropRect.left < 0
                    ...
                    LogUtil.d("nimada#doapply", "handleCropWindowChanged::1786");
                ...
                if (newZoom > 0 && newZoom != mZoom) {
                    if (animate) {
                        ...
                        /* ADDED BY DAIPENGFEI on 10/14/19. */
                        mAnimation.setOnCropImageAnimationEnd(onCropImageAnimationEnd);
                        /* ADDED END. */
                    }

                    mZoom = newZoom;
                    LogUtil.d("nimada#doapply", "handleCropWindowChanged::1828");
                    applyImageMatrix(width, height, true, animate);
                }

            } /* ADDED BY DAIPENGFEI on 10/14/19. */ else if (mZoom == 1 && onCropImageAnimationEnd != null) {
                onCropImageAnimationEnd.run();
            } /* ADDED END. */
    ...
    private void applyImageMatrix(float width, float height, boolean center, boolean animate) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            /*
             * To preventing this method being invoked again when the CropAnimation
             * is running.
             *
             * The CropWindow' rect is displayed well most of time.
             *
             * Sometime the matrix & invertedMatrix can get wrong crop rect from
             * the windowHandler which is being continuely set by the 'cropAnimation'.
             *
             * Normally, it will not happen.
             * On some Phone with a hide(able) virtual keys bar Such as  Smartisan OS on JianGuo p-
             * hone, with this kindof os and a full screen CropImageView. It does happen.
             *
             */
            if (mAnimation != null && mAnimation.isAnimationStarted()) {
                return;
            }
            /* ADDED END. */


            /* ADDED BY DAIPENGFEI on 2020-02-15. */
            final List<RectF> allRects = mCropOverlayView.getAllCropWindowRects();
            /* ADDED END. */

            //LogUtil.d("nimada#applyImageMatrix", "beforeInvert:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            mImageMatrix.invert(mImageInverseMatrix);
            //LogUtil.d("nimada#applyImageMatrix", "afterInvert:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            RectF cropRect = mCropOverlayView.getCropWindowRect();
            //LogUtil.d("nimada#applyImageMatrix", "CropRect:: " + cropRect.toShortString());
            mImageInverseMatrix.mapRect(cropRect);
            for (RectF f : allRects) {
                if (f != cropRect) {
                    mImageInverseMatrix.mapRect(f);
                }
            }
            //LogUtil.d("nimada#applyImageMatrix", "CropRect:: " + cropRect.toShortString());
            mImageMatrix.reset();
            ...
            //LogUtil.d("nimada#applyImageMatrix", "beforeTranslate1:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            mImageMatrix.postTranslate(
                    (width - mBitmap.getWidth()) / 2, (height - mBitmap.getHeight()) / 2);
            //LogUtil.d("nimada#applyImageMatrix", "afterTranselate1:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            ...
            if (mDegreesRotated > 0) {
                ...
                //LogUtil.d("nimada#applyImageMatrix", "afterScaled1:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
                mapImagePointsByImageMatrix();
            }
            ...
            //System.out.println("CropImageView#applyImageMatrix width/bitWidth is " + width + ":" + BitmapUtils.getRectWidth(mImagePoints));
            //System.out.println("CropImageView#applyImageMatrix height/bitHeight is " + height  + ":" + BitmapUtils.getRectHeight(mImagePoints));
            DisplayMetrics dm = getContext().getResources().getDisplayMetrics();
            //System.out.println("CropImageView#applyImageMatrix src_width is " + dm.widthPixels + "," + getMeasuredWidth() + " | " + (mImageView == null ? 0 : mImageView.getMeasuredWidth()));
            //System.out.println("CropImageView#applyImageMatrix src_height is " + dm.heightPixels + "," + getMeasuredHeight() + " | " + (mImageView == null ? 0 : mImageView.getMeasuredHeight()));
            //System.out.println("CropImageView#applyImageMatrix scale is " + scale);
            ...
            if (mScaleType == ScaleType.FIT_CENTER
                ...
                mImageMatrix.postScale(
                        ...
                        BitmapUtils.getRectCenterX(mImagePoints),
                        BitmapUtils.getRectCenterY(mImagePoints));
                //LogUtil.d("nimada#applyImageMatrix", "afterScaled2:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
                mapImagePointsByImageMatrix();
            }
            ...
            mImageMatrix.postScale(
                    ...
                    BitmapUtils.getRectCenterY(mImagePoints));
            mapImagePointsByImageMatrix();
            //LogUtil.d("nimada#applyImageMatrix", "afterScaled3:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            //LogUtil.d("nimada#applyImageMatrix", "CropRect2s:: " + cropRect.toShortString());

            /* MODIFIED BY DAIPENGFEI on 2020-02-15. */
            mImageMatrix.mapRect(cropRect);
            for (RectF f : allRects) {
                if (f != cropRect) {
                    mImageMatrix.mapRect(f);
                }
            }
            /* MODIFIED END. */

            //LogUtil.d("nimada#applyImageMatrix", "CropRect2e:: " + cropRect.toShortString());
            final int gWidth = getWidth();
            final int gHeight = getHeight();
            if (center) {
                ...
                mZoomOffsetX =
                        width > BitmapUtils.getRectWidth(mImagePoints)
                                ...
                                gWidth - BitmapUtils.getRectRight(mImagePoints))
                                / scaleX;
                mZoomOffsetY =
                        height > BitmapUtils.getRectHeight(mImagePoints)
                                ? 0
                                ...
                                gHeight - BitmapUtils.getRectBottom(mImagePoints))
                                / scaleY;
            } else {
            ...
            //LogUtil.d("nimada#applyImageMatrix", "zoom:: " + mZoom + " /scale:: " + scale + " /scaleX :: " + scaleX + " /scaleY:: " + scaleY + " /zoomOffX:: " + mZoomOffsetX + " /zoomOffY:: " + mZoomOffsetY + "/center:: " + center);
            //LogUtil.d("nimada#applyImageMatrix", "zoom:: " + cropRect.toShortString() + "/width:: " + width + " /height:: " + height + " / gWidth:: " + gWidth + " /gHeight:: " + gHeight);
            //LogUtil.d("nimada#applyImageMatrix", "zoom:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            /* MODIFIED BY DAIPENGFEI on 2020-02-15. */
            final float transX = mZoomOffsetX * scaleX;
            final float transY = mZoomOffsetY * scaleY;
            mImageMatrix.postTranslate(transX /*mZoomOffsetX * scaleX*/, transY/*mZoomOffsetY * scaleY*/);
            //LogUtil.d("nimada#applyImageMatrix", "afterTranslate2:: " + mImageMatrix.toShortString() + " == " + mImageInverseMatrix.toShortString());
            for (int i = 0; i < allRects.size(); i++) {
                final RectF f = allRects.get(i);
                if (f == null || f == cropRect) continue;
                f.offset(transX, transY);
                mCropOverlayView.setCropWindowRect(i, f);
            }
            cropRect.offset(transX, transY);
            mCropOverlayView.setCropWindowRect(cropRect);
            /* MODIFIED END. */

            //LogUtil.d("nimada#handler", "/applyImageMatrix::1945/" + animate);
            mapImagePointsByImageMatrix();
            mCropOverlayView.invalidate();
     ...
     * of the image.<br> To get the proper rectangle it must be first reset to original image
     * rectangle.
     */
    private void mapImagePointsByImageMatrix() {
        ...
        LogUtil.d("nimada#mapImagePoints", "imagePoints:: " + mImagePoints[0] + " , " + mImagePoints[1] + ", " + mImagePoints[2] + ", " + mImagePoints[5]);
        LogUtil.d("nimada#mapImagePoints", "imagePointsScaled:: " + mScaleImagePoints[0] + " , " + mScaleImagePoints[1] + ", " + mScaleImagePoints[2] + ", " + mScaleImagePoints[5]);
    }
    ...
    /**
     * Set visibility of crop overlay to hide it when there is no image or specificly set by
     * client.
     */
    ...
    /**
     * Set visibility of progress bar when async loading/cropping is in process and show is
     * enabled.
     */
    ...
    /**
     * The possible cropping area shape.<br> To set square/circle crop shape set aspect ratio to
     * 1:1.
     */
    ...
    /**
     * Options for scaling the bounds of cropping image to the bounds of Crop Image View.<br> Note:
     * Some options are affected by auto-zoom, if enabled.
     */
    public enum ScaleType {

        /**
         * Scale the image uniformly (maintain the image's aspect ratio) to fit in crop image
         * view.<br> The largest dimension will be equals to crop image view and the second
         * dimension will be smaller.
         */
        FIT_CENTER,

        /**
         * Center the image in the view, but perform no scaling.<br> Note: If auto-zoom is enabled
         * and the source image is smaller than crop image view then it will be scaled uniformly to
         * fit the crop image view.
         */
        CENTER,

        /**
         * Scale the image uniformly (maintain the image's aspect ratio) so that both dimensions
         * (width and height) of the image will be equal to or <b>larger</b> than the corresponding
         * dimension of the view (minus padding).<br> The image is then centered in the view.
         */
        CENTER_CROP,

        /**
         * Scale the image uniformly (maintain the image's aspect ratio) so that both dimensions
         * (width and height) of the image will be equal to or <b>less</b> than the corresponding
         * dimension of the view (minus padding).<br> The image is then centered in the view.<br>
    ...
    public enum RequestSizeOptions {
        ...
        /**
         * Only sample the image during loading (if image set using URI) so the smallest of the
         * image dimensions will be between the requested size and x2 requested size.<br> NOTE:
         * resulting image will not be exactly requested width/height see: <a
        ...
        /**
         * Resize the image uniformly (maintain the image's aspect ratio) so that both dimensions
         * (width and height) of the image will be equal to or <b>less</b> than the corresponding
         * requested dimension.<br> If the image is smaller than the requested size it will NOT
         * change.
         */
        RESIZE_INSIDE,
         ...
         * width/height.<br> The largest dimension will be equals to the requested and the second
         * dimension will be smaller.<br> If the image is smaller than the requested size it will
         * enlarge it.
         */
        RESIZE_FIT,

        /**
         * Resize the image to fit exactly in the given width/height.<br> This resize method does
         * NOT preserve aspect ratio.<br> If the image is smaller than the requested size it will
         * enlarge it.
         */
        RESIZE_EXACT
    }
     ...
     * @param l
     */
    public void setCropOpratingStateChangedListener(CropOverlayView.CropOpratingStateChangedListener l) {
        if (mCropOverlayView != null) mCropOverlayView.setCropOpratingStateChangedListener(l);
    }
    ...
    public interface OnSetCropOverlayReleasedListener {
         ...
         *
         * @param rect
         *         The rect coordinates of the cropped overlay
         */
    ...
    public interface OnSetImageUriCompleteListener {
        ...
        /**
         * Called when a crop image view has completed loading image for cropping.<br> If loading
         * failed error parameter will contain the error.
         *
         * @param view
         ...
         * @param error
         *         if error occurred during loading will contain the error, otherwise null.
         */
    ...
    public interface OnCropImageCompleteListener {
        ...
        /**
         * Called when a crop image view has completed cropping image.<br> Result object contains
         * the cropped bitmap, saved cropped image uri, crop points data or the error occured during
         * cropping.
         *
         * @param view
         *         The crop image view that cropping of image was complete.
         * @param result
         *         the crop image result data (with cropped image or error)
         */
    ...
    public static class CropResult {
        ...
        /**
         * The image bitmap of the original image loaded for cropping.<br> Null if uri used to load
         * image or activity result is used.
         */
        private final Bitmap mOriginalBitmap;

        /**
         * The Android uri of the original image loaded for cropping.<br> Null if bitmap was used to
         * load image.
         */
        private final Uri mOriginalUri;

        /**
         * The cropped image bitmap result.<br> Null if save cropped image was executed, no output
         * requested or failure.
         */
        private final Bitmap mBitmap;

        /**
         * The Android uri of the saved cropped image result.<br> Null if get cropped image was
         * executed, no output requested or failure.
         */
        ...
        /**
         * The Android uri of the saved cropped image result Null if get cropped image was executed,
         * no output requested or failure.
         */
        public Uri getUri() {
            return mUri;
        }
