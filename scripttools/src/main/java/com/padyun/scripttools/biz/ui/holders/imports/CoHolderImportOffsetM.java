 ...
 */
public class CoHolderImportOffsetM extends AbsCoHolderImport<CoImportOffset> {
    ...
    public CoHolderImportOffsetM(@NonNull View itemView) {
        super(itemView);
    }
    ...
    protected void onSet(@NonNull Activity act, @NonNull BaseV2RecyclerAdapter adapter, @NonNull CoImportOffset item, int position) {
        ...
        if(data != null){
            ...
            CoBitmapWorker.lruLoad(data.getNonExsitExtraImage().getImageCropPath(), t -> setImageWhenCurItem(mSymbol, t, item));
            CoBitmapWorker.lruLoad(data.getIdentifyImage().getImageCropPath(), bm -> setImageWhenCurItem(mOffsetTar, bm, item), null);
