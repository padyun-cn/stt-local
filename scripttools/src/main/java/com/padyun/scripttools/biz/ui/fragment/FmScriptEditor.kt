 ...
 */
open class FmScriptEditor : FmSimpleList(), ISEActivityAttatchable {
    ...
    companion object {
        ...
        fun newInstance(arg: Bundle?): FmScriptEditor {
            val ins = FmScriptEditor()
            ins.arguments = arg
            return ins
        }
    }
...
//                deleteItem.setBackgroundColor(Color.parseColor("#FF3B32")).setWidth(width).setHeight(height).setImage(R.drawable.ic_delete_menu_project_item)
    ...
    override fun onInitTopView(inflater: LayoutInflater?, topLayout: FrameLayout?) {
        ...
        commonBar?.setRightTextButton("导入") { this@FmScriptEditor.onImportClick() }
    ...
    private fun goCropping() {
        ...
        intent.putExtra("ip", ScriptTestConfig.getServerIp())
        intent.putExtra("asip", ScriptTestConfig.getAsIp())
