 ...
 */
public class CoHolderImportClick extends AbsCoHolderImport<CoImportClick> {
    ...
    public CoHolderImportClick(@NonNull View itemView) {
        super(itemView);
    }
    ...
    protected void onSet(@NonNull Activity act, @NonNull BaseV2RecyclerAdapter adapter, @NonNull CoImportClick importData, int position) {
        ...
        if(data != null) {
            ...
            // load click-image
            CoBitmapWorker.lruLoad(data.getIdentifyImage().getImageCropPath(), t -> setImageWhenCurItem(mSymbol, t, importData));
        }
    }
