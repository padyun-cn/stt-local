 ...
 *
 */
public class TestingSocketThreadBk {
     ...
     * @param asIp
     */
    TestingSocketThreadBk(String ip, int port, String asIp, OnStateChangedListener onStateChangedListener, StatefulWRSockRequest... requests) {
        this(ip, port, asIp, 1000, onStateChangedListener, requests);
    }

    TestingSocketThreadBk(String ip, int port, String asIp, int soTimeout, OnStateChangedListener onStateChangedListener, StatefulWRSockRequest... requests) {
        BSConfiguration configuration = new BSConfiguration();
        configuration.setIp(ip);
        configuration.setPort(StConfiguration.Test.PORT);
